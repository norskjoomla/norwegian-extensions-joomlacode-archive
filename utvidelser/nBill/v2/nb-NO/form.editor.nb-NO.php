...
define("NBILL_FORM_FIELD_DEFAULT_VALUE_HELP", "If you want to pre-populate a value for this field, enter the value here (to select a checkbox, enter the word `On`). Label field types support HTML for this setting. You can also execute PHP code by surrounding it in double dollar signs, for example:<br />\$\$return date(&quot;d/m/Y&quot;);\$\$ (WARNING! Fatal errors in any PHP code you enter will break nBill, and you may need to modify the database directly to remove the offending code! Make sure your code is error-free before saving!)");
