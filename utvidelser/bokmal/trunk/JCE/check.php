...
echo '<title>JCE! - Language file checker</title>';
echo '</head>';
echo '<body>';
...
function checkPhpLngFiles() {
	global $files_php1, $files_php2, $base;
	global $lang1, $lang2;
	
	$files = array();
	
	foreach( $files_php1 as $key => $file ) {
		$tmp = str_replace( $base . DIRECTORY_SEPARATOR, '', dirname( $file ) . DIRECTORY_SEPARATOR );
		$files_php1[$key] = $tmp;
	}
	
	foreach( $files_php2 as $key => $file ) {
		$tmp = str_replace( $base . DIRECTORY_SEPARATOR, '', dirname( $file ) . DIRECTORY_SEPARATOR );
		$files_php2[$key] = $tmp;
	}
	
	$files = array_unique( array_merge( $files_php1, $files_php2 ) );
	...
	*/
	...
	foreach( $files as $file ) {
		if( !in_array( $file, $files_php1 ) ) {
			echo "<tr><td colspan=4><span style=\"font-size: large;\">$file$lang2.php</span>&nbsp;&nbsp;&nbsp;<span style=\"color: red;\">Deprecated</span></td></tr>\n";
		} elseif( !in_array( $file, $files_php1 ) ) {
			echo "<tr><td colspan=4><span style=\"font-size: large;\">$file$lang2.php</span>&nbsp;&nbsp;&nbsp;<span style=\"color: red;\">Missing</span></td></tr>\n";
		} else {
			echo "<tr><td style=\"border: 1px black solid\" colspan=4><span style=\"font-size: large;\">$file$lang2.php</span></td></tr>\n";
			checkPhpLngFile( "$file$lang1.php", "$file$lang2.php" );
		}
		echo "<tr><td colspan=4><span style=\"font-size: large;\"><br/></span></td></tr>\n";
	}
	echo '</table>';
...
function checkPhpLngFile( $file1, $file2 ) {
	
	$language = defineListPhp( $file2 );
	$english = defineListPhp( $file1 );
...
function findLngFiles( $dir ) {
	global $files_js1, $files_php1, $lang1;
	global $files_js2, $files_php2, $lang2;
	...
	while( $file = readdir( $dh ) ) {
		...
		if( ($file == '.') || ($file == '..') ) {
			...
			if( $file == $lang1.'.php' ) { $files_php1[] = $tmp; }
			if( $file == $lang2.'.php' ) { $files_php2[] = $tmp; }
		}
	}
	closedir( $dh );
}
