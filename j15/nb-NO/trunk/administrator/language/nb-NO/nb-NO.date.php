...
class JDate extends JObject
{
	...
	{
		$date = ($this->_date !== false) ? JDate::strftime($format, $this->_date + $this->_offset) : null;
		return $date;
	}
	
	function strftime($format, $time)
	{
		$format = str_replace('%a', JDate::_dayToString(date('w', $time), true), $format);
		$format = str_replace('%A', JDate::_dayToString(date('w', $time)), $format);
		$format = str_replace('%b', JDate::_monthToString(date('n', $time), true), $format);
		$format = str_replace('%B', JDate::_monthToString(date('n', $time)), $format);
